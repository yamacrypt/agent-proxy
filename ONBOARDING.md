...
- `conditionalPassthrough[].hostPatterns` に一致して selector も一致すれば passthrough
- `requestFiltering.allowRules[].hosts` に入った host は rule 優先
- どれにも入っていない host だけ fallback GET が効く
...
2. `conditionalPassthrough[].hostPatterns` と selector に一致すれば passthrough
3. `requestFiltering.allowRules[].hosts` に一致すれば inspect
4. `inspectFallbackAllowedMethods` が非空なら inspect fallback
5. どれにも当たらなければ block

つまり、同じ host を複数箇所に書いたときは `passthroughHosts` が最優先です。
...
特定 selector だけ対象 host を素通ししたい場合は、`conditionalPassthrough` を使います。
AWS profile ごとの passthrough では、proxy URL の Basic auth password 部分に profile 名を載せます。
...
- `allowedPasswords` に一致した AWS host は passthrough
- 一致しない profile は既存ルールで inspect
- `allowedPasswords` は glob として評価されるので、`prod-*` や `*admin*` も使えます
- `onMissingSelector=block` にすると selector なし対象 host の通信を拒否
