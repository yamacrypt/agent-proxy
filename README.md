...
```bash
cp config/proxy.config.example.json config/proxy.config.json
```
...
`hosts` を省略した allow rule は全 host を inspect 対象にします。たとえば `Authorization` header に `accessToken=1111` が含まれる request だけ全 method で許可するなら、このように書けます。
...
2. `allowRules[].hosts` に一致したら `inspect`
3. `hosts` を省略した `allowRules[]` があれば `inspect`
...
2. allow rule に一致したら allow
3. 明示 rule のない host では fallback method を allow
4. それ以外は block
...
config 読み込み、rule 評価、`config/proxy.config.example.json` の挙動をテストしています。
